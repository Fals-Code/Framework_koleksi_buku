    ...
    <div class="search-field d-none d-md-block">
      <form class="d-flex align-items-center h-100" action="#">
        <div class="input-group">
          <div class="input-group-prepend bg-transparent"><i class="input-group-text border-0 mdi mdi-magnify text-muted"></i></div>
    ...
    <ul class="navbar-nav navbar-nav-right">
        ...
        <div class="dropdown-menu dropdown-menu-right navbar-dropdown preview-list shadow-lg border-0" aria-labelledby="notificationDropdown" style="width: 350px; border-radius: 15px;">
          ...
          @if(auth()->user()->notifications->count() > 0)
            <div class="p-2 text-center">
                <form action="{{ route('notifications.clear') }}" method="POST">@csrf<button type="submit" class="btn btn-link btn-sm text-primary text-decoration-none fw-bold">Bersihkan Semua Notifikasi</button></form>
            </div>
          @endif
        </div>
      </li>
      <li class="nav-item nav-profile dropdown">
        <a class="nav-link dropdown-toggle" id="profileDropdown" href="#" data-bs-toggle="dropdown">
          ...
          <div class="nav-profile-text"><p class="mb-1 text-black fw-bold">{{ Auth::user()->name }}</p></div>
        </a>
